...
type HaskifyAction = State.StateT (Token,T.Text) (MaybeT IO)

haskifyLiftMaybe :: Maybe a -> HaskifyAction a
haskifyLiftMaybe = lift . MaybeT . return

data CurrentPlayback = CurrentPlayback {
  device_name :: T.Text,
  current_playlist :: T.Text,
  current_track :: T.Text,
  current_name :: T.Text,
  is_playing :: Bool
} deriving (Show)

instance FromJSON CurrentPlayback where
  parseJSON = withObject "CurrentPlayback" $ \v -> do
    device  <- v .: "device"
    device_name <- device .: "name"

    context <- v .: "context"
    current_playlist <- context .: "uri"

    item    <- v .: "item"
    current_track <- item .: "name" 

    playing_track_artists <- head <$> item .: "artists"
    current_name <-  playing_track_artists .: "name"
